...
export async function triage(org_id: string, message: string) {
  ...
  await supabase.from('ai_logs').insert({ id: crypto.randomUUID(), org_id, prompt_hash: 'stub', response_id: crypto.randomUUID(), llm_model: 'stub', tokens_used: 16, meta: { message } });
